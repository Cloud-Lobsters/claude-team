...
> Do **not** rely on the `$page` store in `load` functions, as it may not be populated for certain requests.
...
* The `AUTH_TRUST_HOST` flag should be set to `true` when deploying outside Vercel (e.g., Cloudflare Pages, Netlify).
