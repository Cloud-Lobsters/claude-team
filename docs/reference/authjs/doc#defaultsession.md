...
| **Account** | Holds provider account data (OAuth tokens, etc.). | `access_token?`, `expires_at?`, `provider`, `providerAccountId`, `userId?` |
| **DefaultSession** | Base session shape. | `expires`, `user?` |
| **Profile** | User info returned by OAuth providers. | `email?`, `name?`, `picture?`, … |
| **Session** | Active session of the logged‑in user. | `expires`, `user?` |
| **User** | Shape of the returned object in the provider’s `profile` callback. | `email?`, `id?`, `image?`, `name?` |

---

## 9. Custom fetch

If you need to use a corporate proxy or a custom fetch implementation:
...
*`config`* can be a plain `SvelteKitAuthConfig` or an async function that receives the SvelteKit `event`.
...
4. Add `signIn` / `signOut` form‑action routes.
5. (Optional) Add layout load to expose session.
6. Protect routes via `load` or middleware.
