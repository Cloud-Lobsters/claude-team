...
|---------|------|---------|
| Install | `package.json` | `npm install @auth/sveltekit` |
| Basic config | `src/auth.ts` | `SvelteKitAuth({ providers: [GitHub] })` |
| Lazy init | `src/auth.ts` | `SvelteKitAuth(async (event) => { ... })` |
| Sign‑in component | `src/routes/+page.svelte` | `<SignIn>…</SignIn>` |
| Sign‑out component | `src/routes/+page.svelte` | `<SignOut>…</SignOut>` |
| Client helpers | `@auth/sveltekit/client` | `signIn("github")` |
| Session load | `src/routes/+layout.server.ts` | `event.locals.auth()` |
| Page protection | `src/routes/protected/+page.server.ts` | `redirect(303, "/auth")` |
| Path protection | `src/hooks.server.ts` | `sequence(authenticationHandle, authorizationHandle)` |
