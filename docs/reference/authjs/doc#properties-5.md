...
> Do **not** rely on `$page.data.session` in `load` because the layout load may not run for that request.
...
* The `AUTH_SECRET` must be a 32‑character random string.  
...
Base error class for all Auth.js errors. It prints nicely in server logs.
...
Represents an OAuth account. Extends `TokenSet` and contains provider‑specific fields.
...
6. Protect routes via `load` or `handle` middleware.  
