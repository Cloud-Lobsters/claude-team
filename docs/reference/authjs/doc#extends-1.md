...
> It may not run for all leaf routes, leading to accidental exposure.

### 6.2 Per‑path (middleware style)
...
4. Add `AUTH_SECRET` (and provider secrets) to your environment.
5. Use `<SignIn />` / `<SignOut />` components or client‑side helpers.
6. Protect routes with `load` functions or a middleware `handle` hook.
