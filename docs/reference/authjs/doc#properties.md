...
> On Vercel it is automatically injected; on other hosts set `AUTH_TRUST_HOST=true`.
...
When environment variables are only available at request time (e.g. Cloudflare Workers), use a function that receives the `event`:
...
Exported helpers from `@auth/sveltekit/client` allow you to trigger sign‑in/out without form actions.
...
> Using `$page.data.session` can lead to race conditions if the layout load is skipped.

### 6.2 Per‑path (middleware)

Override the `handle` hook to protect a whole path:
...
* When prerendering, avoid hard‑coded `<a href="/auth/signin">` links; use the provided components or client helpers instead.
* The `AUTH_TRUST_HOST` flag should be set to `true` on non‑Vercel hosts (e.g., Cloudflare Pages, Netlify).
...
*Redirects to `/auth/signin?error=CredentialsSignin&code=credentials`.*
...
If you need to override the default `fetch` used by providers (e.g., corporate proxy), use the `customFetch` symbol:
...
| Lazy init | `SvelteKitAuth(async (event) => { /* read env */ })` |
| Sign‑in component | `<SignIn>…</SignIn>` |
| Sign‑out component | `<SignOut>…</SignOut>` |
| Client helper | `signIn("github")` |
