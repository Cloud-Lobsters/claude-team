...
Useful when environment variables are available only at runtime (e.g. Cloudflare Workers).
...
* When prerendering, avoid using `<a href="/auth/signin">` – use the built‑in components or client‑side helpers instead.
...
| **User** | Shape of the returned object in provider callbacks. | `email?`, `id?`, `image?`, `name?` |
...
That’s it! You now have a fully‑functional authentication system in SvelteKit powered by Auth.js. Happy coding!