...
| **Profile** | User info returned by OAuth provider. | `address?`, `birthdate?`, `email?`, `email_verified?`, `family_name?`, `gender?`, `given_name?`, `id?`, `locale?`, `middle_name?`, `name?`, `nickname?`, `phone_number?`, `picture?`, `preferred_username?`, `profile?`, `sub?`, `updated_at?`, `website?`, `zoneinfo?` |
...
If you need to override the default fetch (e.g., corporate proxy, custom headers):
...
4. Add `signIn`/`signOut` actions or use the provided components.
5. (Optional) Add a layout load to expose the session.
6. Protect routes via `+page.server.ts` or the handle hook.
