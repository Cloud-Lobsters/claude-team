...
> Using the `$page` store can lead to race conditions.
...
4. Add `AUTH_SECRET` (32+ chars) to your environment.
5. Use `<SignIn />` / `<SignOut />` components or client‑side helpers.
6. (Optional) Add a layout load to expose the session.
7. Protect routes via `load` or a middleware `handle`.
