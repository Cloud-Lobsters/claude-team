...
The returned object is available in `$page.data` across the app.
...
> **Never** put authorization logic in a `+layout.server.ts`.  
> Use `event.parent()` only when you’re sure the layout load has run.
