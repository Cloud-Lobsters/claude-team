...
* `Session` extends `DefaultSession` and contains `expires` and optional `user`.
* `User` extends `DefaultUser` and may include `email`, `id`, `image`, `name`.
* `Profile` contains standard OpenID Connect claims (e.g., `email`, `name`, `picture`).
