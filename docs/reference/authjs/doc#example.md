...
*If thrown in a framework that handles form actions server‑side, the error is thrown instead of redirecting.*
...
See the [official guide](https://authjs.dev/guides/corporate-proxy) for more details.
...
5. Expose the session via a layout load.  
6. Protect routes with `load` or a custom `handle` hook.  
