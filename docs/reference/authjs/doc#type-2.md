...
Use the `handle` hook to protect a whole URI segment:
...
| Route protection | `if (!session?.user) throw redirect(303, "/auth")` |
| Handle hook | `sequence(authenticationHandle, authorizationHandle)` |
