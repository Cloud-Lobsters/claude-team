...
* When prerendering, avoid using `<a href="/auth/signin">` – use the provided components or client helpers instead.
...
|------|---------|----------------|
| `Account` | `Partial<TokenEndpointResponse>` | `access_token`, `expires_at`, `id_token`, `provider`, `providerAccountId`, `refresh_token`, `scope`, `token_type`, `type`, `userId` |
...
If you need to override the fetch used by providers (e.g., corporate proxy), use the `customFetch` symbol:
