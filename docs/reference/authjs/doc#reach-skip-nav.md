...
> **Important** – Always read the session from `event.locals.auth()` inside the load function; do not rely on the `$page` store.
...
| `User` | User profile returned by provider |
...
If you need to use a corporate proxy or custom fetch implementation:
...
**Enjoy building secure SvelteKit apps with Auth.js!**