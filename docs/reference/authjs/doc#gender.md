...
> **Tip** – Always fetch the session from `event.locals.auth()` inside `load`. Do **not** rely on `$page.data.session` in `load` functions.
...
| Type | Description |
|------|-------------|
| `Account` | OAuth provider account details (extends `TokenSet`). |
...
## 10. Example: Custom Fetch (Corporate Proxy)
...
- [SvelteKit Handle Hook Docs](https://kit.svelte.dev/docs/hooks#handle)  
- [SvelteKit Sequence Middleware](https://kit.svelte.dev/docs/hooks#sequence)  
