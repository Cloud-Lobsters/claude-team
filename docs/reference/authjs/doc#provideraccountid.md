...
| **SignInError** | Base for sign‑in errors. | `code` (query param) |
...
Standard OpenID Connect claims (e.g. `email`, `name`, `picture`, etc.).
...
4. **Use** `<SignIn />` / `<SignOut />` components or client‑side helpers.  
5. **Inject** session via a layout load.  
6. **Protect** routes either per‑page or via a handle hook.  
