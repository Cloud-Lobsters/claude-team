...
The callback URL for each provider defaults to:
...
> **Important** – Always read the session from `event.locals.auth()` inside the load function; do not rely on the `$page` store for server‑side protection.
...
* When prerendering, avoid using `<a href="/auth/signin">` links; use the built‑in components or client helpers instead.
...
### 8.4 `DefaultSession`, `Session`, `User`, `Profile`

* `DefaultSession` – `{ expires: string; user?: User }`
* `Session` – extends `DefaultSession`
* `User` – `{ id?: string; name?: string; email?: string; image?: string }`
* `Profile` – OpenID Connect standard claims (address, email, name, picture, etc.)
...
2. Create `src/auth.ts` with providers and export `handle`, `signIn`, `signOut`.
