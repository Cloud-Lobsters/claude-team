...
> Do **not** rely on the `$page` store in `load` functions.
...
Now every route under `/authenticated` is protected.
...
Re‑exports the core `SvelteKitAuthConfig` type. See the Auth.js docs for full options.
