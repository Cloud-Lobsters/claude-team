...
User info returned by OAuth providers (OpenID Connect claims).
...
{#if $page.data.session}
  ...
  <SignOut><button>Sign out</button></SignOut>
{:else}
  <SignIn><button>Sign in</button></SignIn>
{/if}
```

That’s it! You now have a fully‑functional authentication system in SvelteKit. Happy coding!