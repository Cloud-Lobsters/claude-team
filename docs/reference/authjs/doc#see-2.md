...
The callback URL for providers must be:
...
> **Never** put auth logic in a `+layout.server.ts`.  
> Use `event.locals.auth()` inside each page’s load to guarantee protection.
...
* When prerendering, avoid `<a href="/auth/signin">` links; use the provided components or client‑side helpers.
* The `AUTH_TRUST_HOST` flag is required for non‑Vercel hosts (Cloudflare Pages, Netlify, etc.).
...
| Type | Description |
|------|-------------|
| `Account` | OAuth account data (extends `TokenSet`) |
| `DefaultSession` | Base session shape |
| `Session` | Active session (extends `DefaultSession`) |
| `User` | User profile returned by providers |
| `Profile` | OpenID Connect standard claims |
...
* `config` – `SvelteKitAuthConfig` or a function returning it.

### `SvelteKitAuthConfig`

Re‑exports the core Auth.js config with SvelteKit‑specific additions.
...
**Enjoy secure authentication in your SvelteKit app!**