...
> Example (Unix): `openssl rand -hex 32`  
...
> **Tip** – Always fetch the session from `event.locals.auth()` inside `load`. Do **not** rely on the `$page` store in `load` functions.
...
- The `AUTH_TRUST_HOST` variable should be set to `true` on non‑Vercel hosts (e.g., Cloudflare Pages, Netlify).
...
- [Auth.js Docs – Corporate Proxy](https://authjs.dev/guides/corporate-proxy)  
- [SvelteKit Docs – Load Functions](https://kit.svelte.dev/docs/load)  
- [SvelteKit Docs – Handle Hook](https://kit.svelte.dev/docs/hooks#handle)  

---

**© 2025 Auth.js – Balázs Orbán & Team**  
[GitHub](https://github.com/authjs) | [NPM](https://www.npmjs.com/package/@auth/sveltekit) | [Discord](https://discord.gg/authjs)